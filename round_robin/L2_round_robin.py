...
class L2RoundRobin:
    """
    A class representing a round-robin match between two L2 bots' responses.
    
    This class holds a pair of L2 bot instances (which share the same instruction)
    and is tasked with comparing their responses (stored in the attribute `section_draft`)
    to decide which response is better. The comparison is done asynchronously using llm_call.
    
    Attributes:
        bot1: The first L2 bot instance.
        bot2: The second L2 bot instance.
    ...
    """
    def __init__(self, bot1: L2Bot, bot2: L2Bot):
        self.bot1 = bot1
        self.bot2 = bot2
        self.result = None
        ...
        self.system_prompt_reasoning = (
            "You are a Level 2 (L2) Evaluation Bot. Your task is to critically assess two "
            "responses generated by L2 bots. First, provide a detailed reasoning about the strengths "
        ...
        self.system_prompt_decision = (
            "You are a Level 2 (L2) Evaluation Bot. Based on the provided reasoning, decide which response is superior. "
    ...
    async def _reason_competition(self):
        ...
        prompt = (
            ...
            f"{self.bot2.section_draft}\n\n"

            "You have been provided with the responses of two L2 bots above, i.e., "
            "section block 1 and section block 2. Reason about the strengths and weaknesses of "
            "each in terms of the quality of the section and adherence to the instructions. "
    ...
    async def _final_decision(self):
        ...
        prompt = (
            ...
            "You have been provided with the responses of two L2 bots above, i.e., "
            "section block 1 and section block 2. You just reasoned about the strengths and weaknesses of "
            "each in terms of the quality of the section and adherence to the instructions. "
    ...
    async def step(self):
        ...
        if self.done:
            raise RuntimeError("L2 round robin bot called after being marked done.")
        if self.current_llm_call_index >= len(llm_call_sequence):
            raise RuntimeError("L2 round robin bot invalid call sequence index.")
